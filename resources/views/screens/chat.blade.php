...
@php
    $initial = [
        ...
        'system' => 'You are a helpful, articulate assistant. Provide clear, structured answers. When appropriate, use markdown formatting including headings, lists, and code blocks.',
        'provider' => null,
        'model' => null,
        'temperature' => 0.7,
        'maxTokens' => 4096,
...
<x-app.page-container class="flex h-[calc(100dvh-4rem)] flex-col">
    <div x-data="pfChat({{ Js::from($initial) }})" x-cloak class="flex min-h-0 flex-1 flex-col">
        ...
        {{-- Header --}}
        <header class="flex shrink-0 items-center justify-between gap-4 border-b border-zinc-200 px-5 py-3 dark:border-white/10">
            <div class="min-w-0">
                <p class="mb-0.5 text-xs font-semibold uppercase tracking-widest text-brand-600 dark:text-brand-400">Chat</p>
                <div class="flex items-center gap-2">
                    <h1 class="truncate text-lg font-semibold tracking-tight text-zinc-900 dark:text-white">Conversation</h1>
                    <span class="hidden items-center gap-1.5 rounded-full bg-zinc-100 px-2 py-0.5 text-xs font-medium tabular-nums text-zinc-500 sm:inline-flex dark:bg-white/5 dark:text-zinc-400"
        ...
        <div class="flex min-h-0 flex-1 overflow-hidden">
            ...
            <div class="flex min-h-0 min-w-0 flex-1 flex-col">
                ...
                <div x-ref="thread" class="flex-1 overflow-y-auto px-4 py-6 sm:px-6">
                    ...
                    <template x-if="messages.length === 0">
                        <div class="flex flex-col items-center justify-center gap-4 py-20 text-center">
                            <div class="grid size-14 place-items-center rounded-2xl bg-zinc-100 text-zinc-400 dark:bg-white/5 dark:text-zinc-500">
                                <flux:icon.chat-bubble-left-right class="size-6" />
                            </div>
                            <div>
                                <p class="text-sm font-medium text-zinc-700 dark:text-zinc-200">Start a conversation</p>
                                <p class="mt-1 max-w-sm text-xs leading-relaxed text-zinc-400 dark:text-zinc-500">
                                    Ask questions, get explanations, write code, or brainstorm ideas. Your conversation history stays in this session.
                                </p>
                            </div>
                        </div>
                    </template>
                            ...
                            <div x-data="{ show: false }" x-init="$nextTick(() => show = true)" x-show="show"
                                ...
                                <template x-if="msg.role === 'user'">
                                    <div class="max-w-[85%] sm:max-w-[75%]">
                                        <div class="rounded-2xl rounded-br-md bg-brand-600 px-4 py-3 text-sm leading-relaxed text-white dark:bg-brand-500">
                                            <p class="whitespace-pre-wrap" x-text="msg.content"></p>
                                        </div>
                                        <p class="mt-1.5 text-right text-[0.65rem] text-zinc-400 dark:text-zinc-500" x-text="timeAgo(msg.timestamp)"></p>
                                    </div>
                                </template>
                                ...
                                <template x-if="msg.role === 'assistant'">
                                    <div class="max-w-[85%] sm:max-w-[85%]">
                                        <div class="flex items-start gap-3">
                                            {{-- Avatar --}}
                                            <div class="mt-0.5 flex size-7 shrink-0 items-center justify-center rounded-full bg-zinc-900 text-[0.6rem] font-bold text-white dark:bg-zinc-700">
                                                AI
                                            </div>
                                            <div class="min-w-0 flex-1">
                                                <div class="rounded-2xl rounded-tl-md border border-zinc-200 bg-white px-4 py-3 shadow-xs dark:border-white/10 dark:bg-zinc-900/60">
                                                    {{-- Streaming cursor --}}
                                                    <template x-if="msg.status === 'streaming'">
                                                        <div>
                                                            <div class="prose-chat prose prose-sm dark:prose-invert max-w-none">
                                                                <div class="whitespace-pre-wrap text-sm leading-relaxed text-zinc-700 dark:text-zinc-200">
                                                                    <span x-text="msg.content"></span><span class="animate-caret inline-block h-[0.9em] w-[2px] translate-y-[0.1em] bg-brand-500"></span>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </template>
                                                    {{-- Completed content --}}
                                                    <template x-if="msg.status === 'completed' || msg.status === 'cancelled'">
                                                        <div>
                                                            <div class="prose-chat prose prose-sm dark:prose-invert max-w-none">
                                                                <div class="whitespace-pre-wrap text-sm leading-relaxed text-zinc-700 dark:text-zinc-200" x-html="renderMarkdown(msg.content)"></div>
                                                            </div>
                                                            <template x-if="msg.status === 'cancelled'">
                                                                <div class="mt-2 rounded-md border border-dashed border-zinc-200 px-3 py-1.5 text-[0.65rem] text-zinc-400 dark:border-white/10 dark:text-zinc-500">
                                                ...
                                                <template x-if="msg.status === 'completed' && msg.tokensOut">
                                                    <div class="mt-2 flex flex-wrap items-center gap-2 text-[0.65rem] tabular-nums text-zinc-400 dark:text-zinc-500">
                                                        <span class="inline-flex items-center gap-1 rounded-md border border-zinc-200 bg-zinc-50 px-1.5 py-0.5 dark:border-white/10 dark:bg-white/5">
                                                            <span class="font-mono text-zinc-500 dark:text-zinc-400" x-text="formatTokens(msg.tokensIn)"></span> in
                                                        </span>
                                                        <span class="inline-flex items-center gap-1 rounded-md border border-zinc-200 bg-zinc-50 px-1.5 py-0.5 dark:border-white/10 dark:bg-white/5">
                                                            <span class="font-mono text-zinc-500 dark:text-zinc-400" x-text="formatTokens(msg.tokensOut)"></span> out
                                                        </span>
                                                        <span class="inline-flex items-center gap-1 rounded-md border border-zinc-200 bg-zinc-50 px-1.5 py-0.5 dark:border-white/10 dark:bg-white/5">
                                                            $<span class="font-mono text-zinc-500 dark:text-zinc-400" x-text="(msg.cost || 0).toFixed(4)"></span>
                                                        </span>
                ...
                {{-- Input area --}}
                <div class="shrink-0 border-t border-zinc-200 bg-zinc-50/80 px-4 py-3 backdrop-blur dark:border-white/10 dark:bg-zinc-950/80 sm:px-6">
                    <div class="mx-auto max-w-3xl">
                        <div class="flex items-end gap-3 rounded-2xl border border-zinc-200 bg-white p-2 shadow-sm transition focus-within:border-brand-500/50 focus-within:ring-2 focus-within:ring-brand-500/20 dark:border-white/10 dark:bg-zinc-900/60 dark:focus-within:border-brand-400/50 dark:focus-within:ring-brand-400/20">
                            <textarea x-ref="chatInput" x-model="input" x-on:keydown="handleKeydown($event)" x-on:input="autoResize($event)"
                                rows="1" placeholder="Type a message... (Shift+Enter for newline)"
                        ...
                        <div class="mt-2 flex items-center justify-between text-[0.65rem] text-zinc-400 dark:text-zinc-500">
                            <span>
                                <kbd class="rounded border border-zinc-200 bg-white px-1 py-0.5 font-mono text-[0.6rem] dark:border-white/10 dark:bg-white/5">Enter</kbd>
                                to send
                                <span class="mx-1">&middot;</span>
                                <kbd class="rounded border border-zinc-200 bg-white px-1 py-0.5 font-mono text-[0.6rem] dark:border-white/10 dark:bg-white/5">Shift+Enter</kbd>
                                for newline
                            </span>
                            <span x-show="isStreaming" class="flex items-center gap-1.5 text-brand-500 dark:text-brand-400">
                ...
                <div class="space-y-5 p-4">
                    ...
                    <div>
                        <button type="button" x-on:click="showSystemPrompt = !showSystemPrompt"
                            ...
                            <flux:icon.chevron-down class="size-3.5 transition-transform" :class="showSystemPrompt ? 'rotate-180' : ''" />
                        </button>
                    ...
                    <div>
                        ...
                        <div class="mt-2 flex flex-wrap items-center gap-x-3 gap-y-1 text-[0.65rem] text-zinc-400 dark:text-zinc-500">
                            <span class="inline-flex items-center gap-1">
                                <flux:icon.window class="size-3" />
                                <span class="font-mono text-zinc-500 dark:text-zinc-400" x-text="(currentModel.context || 0).toLocaleString()"></span> ctx
                            </span>
                            <span class="inline-flex items-center gap-1">
                                <flux:icon.arrow-right-start-on-rectangle class="size-3" />
                                <span class="font-mono text-zinc-500 dark:text-zinc-400" x-text="formatPrice((currentModel.pricing || {}).input || 0)"></span>
                            </span>
                            <span class="inline-flex items-center gap-1">
                                <flux:icon.arrow-right-end-on-rectangle class="size-3" />
                                <span class="font-mono text-zinc-500 dark:text-zinc-400" x-text="formatPrice((currentModel.pricing || {}).output || 0)"></span>
                            </span>
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="range" min="0" max="2" step="0.05" x-model.number="temperature" :disabled="isStreaming"
                        ...
                        <div>
                            ...
                            <input type="range" min="0" max="1" step="0.05" x-model.number="topP" :disabled="isStreaming"
                                class="mt-2 w-full cursor-pointer accent-brand-600 disabled:opacity-60" />
                        </div>

                        <div>
                            <label class="text-xs font-medium text-zinc-400">Max tokens</label>
                            <input type="number" min="0" step="256" x-model.number="maxTokens" :disabled="isStreaming"
                        ...
                        <div class="flex items-center justify-between gap-3">
                            <div>
                                ...
                                <p class="text-[0.65rem] text-zinc-400 dark:text-zinc-500">Receive tokens as generated.</p>
                            </div>
                            <button type="button" role="switch" x-on:click="stream = !stream" :disabled="isStreaming"
                                :aria-checked="stream"
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            <div class="rounded-lg border border-zinc-200 bg-zinc-50 px-3 py-2 dark:border-white/10 dark:bg-white/5">
                                <p class="text-[0.65rem] text-zinc-400 dark:text-zinc-500">Total in</p>
                                <p class="font-mono text-sm font-semibold tabular-nums text-zinc-700 dark:text-zinc-200" x-text="formatTokens(totalTokensIn)"></p>
                            </div>
                            <div class="rounded-lg border border-zinc-200 bg-zinc-50 px-3 py-2 dark:border-white/10 dark:bg-white/5">
                                <p class="text-[0.65rem] text-zinc-400 dark:text-zinc-500">Total out</p>
                                <p class="font-mono text-sm font-semibold tabular-nums text-zinc-700 dark:text-zinc-200" x-text="formatTokens(totalTokensOut)"></p>
                            </div>
                            <div class="col-span-2 rounded-lg border border-zinc-200 bg-zinc-50 px-3 py-2 dark:border-white/10 dark:bg-white/5">
                                <p class="text-[0.65rem] text-zinc-400 dark:text-zinc-500">Estimated cost</p>
                                <p class="font-mono text-sm font-semibold tabular-nums text-brand-600 dark:text-brand-400">$<span x-text="totalCost.toFixed(4)"></span></p>
                            </div>
                        </div>
                    </div>
                </div>
...
document.addEventListener('alpine:init', () => {
    Alpine.magic('markdown', () => {
        return (text) => window.__pfMarkdown?.(text) || text;
    });
});

// Expose globally so the component can call it
window.renderMarkdown = function (text) {
    if (!text) return '';
    let html = text
        // Code blocks
        .replace(/```(\w*)\n([\s\S]*?)```/g, '<pre class="rounded-lg bg-zinc-100 px-3.5 py-3 font-mono text-xs leading-relaxed text-zinc-700 dark:bg-zinc-800 dark:text-zinc-200"><code>$2</code></pre>')
        // Inline code
        .replace(/`([^`]+)`/g, '<code class="rounded-md bg-zinc-100 px-1.5 py-0.5 font-mono text-xs text-brand-700 dark:bg-zinc-800 dark:text-brand-300">$1</code>')
        // Bold
        .replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>')
        // Italic
        .replace(/\*(.+?)\*/g, '<em>$1</em>')
        // Headings
        .replace(/^### (.+)$/gm, '<h3 class="mt-4 mb-1 text-sm font-semibold text-zinc-900 dark:text-white">$1</h3>')
        .replace(/^## (.+)$/gm, '<h2 class="mt-5 mb-2 text-base font-semibold text-zinc-900 dark:text-white">$1</h2>')
        .replace(/^# (.+)$/gm, '<h1 class="mt-6 mb-2 text-lg font-bold text-zinc-900 dark:text-white">$1</h1>')
        // Horizontal rules
        .replace(/^---$/gm, '<hr class="my-4 border-zinc-200 dark:border-white/10" />')
        // Unordered lists
        .replace(/^- (.+)$/gm, '<li class="ml-4 list-disc text-sm text-zinc-700 dark:text-zinc-200">$1</li>')
        // Ordered lists
        .replace(/^\d+\. (.+)$/gm, '<li class="ml-4 list-decimal text-sm text-zinc-700 dark:text-zinc-200">$1</li>')
        // Tables (basic)
        .replace(/\|(.+)\|/g, function(match) {
            const cells = match.split('|').filter(c => c.trim());
            if (cells.every(c => /^[\s-:]+$/.test(c))) return '<!-- table-sep -->';
            const tag = 'td';
            return '<tr>' + cells.map(c => `<${tag} class="px-3 py-1.5 text-sm text-zinc-700 dark:text-zinc-200">${c.trim()}</${tag}>`).join('') + '</tr>';
        })
    ...
    // Clean up table separator rows and wrap tables
    html = html.replace(/<!-- table-sep -->/g, '');

    // Wrap consecutive <tr> in <table>
    html = html.replace(/(<tr>.*?<\/tr>)+/gs, '<table class="my-3 w-full border-collapse rounded-lg border border-zinc-200 dark:border-white/10">$&</table>');
