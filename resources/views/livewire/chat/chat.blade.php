<div x-data="pfChat()" class="mx-auto flex h-[calc(100dvh-4rem)] w-full max-w-[1400px] flex-col px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
        ...
        <div class="min-w-0">
            ...
            <div class="flex items-center gap-2">
                <h1 class="truncate text-lg font-semibold tracking-tight text-zinc-900 dark:text-white">
                    Conversation</h1>
                <span
                    class="hidden items-center gap-1.5 rounded-full bg-zinc-100 px-2 py-0.5 text-xs font-medium tabular-nums text-zinc-500 sm:inline-flex dark:bg-white/5 dark:text-zinc-400">{{ count($messages) }} messages</span>
            </div>
        </div>
        <div class="flex items-center gap-2">
    ...
    <div class="flex min-h-0 flex-1 overflow-hidden">
                    ...
                    <div class="mx-auto max-w-3xl space-y-6">
                        @foreach ($messages as $message)
                            <div wire:key="chat-message-{{ $message['id'] }}"
                                x-data="{ show: false }" x-init="$nextTick(() => show = true)" x-show="show"
                                ...
                                @if ($message['role'] === 'user')
                                    <div class="max-w-[85%] sm:max-w-[75%]">
                                        ...
                                        <p class="mt-1.5 text-right text-[0.65rem] text-zinc-400 dark:text-zinc-500">
                                            {{ \App\Support\MockData::timeAgo($message['timestamp']) }}</p>
                                    </div>
                                @endif
                                        ...
                                        <div class="flex items-start gap-3">
                                            ...
                                            <div class="min-w-0 flex-1">
                                                <div
                                                    ...
                                                    @if (($message['status'] ?? null) === 'streaming')
                                                        <div class="prose-chat prose prose-sm dark:prose-invert max-w-none">
                                                            <div
                                                                ...
                                                                <span id="pf-stream-{{ $message['id'] }}">{{ $message['content'] }}</span>
                                                                <span
                                                    ...
                                                    @else
                                                        <div class="prose-chat prose prose-sm dark:prose-invert max-w-none">
                                                            <div class="whitespace-pre-wrap text-sm leading-relaxed text-zinc-700 dark:text-zinc-200"
                                                                x-html="renderMarkdown({{ Js::from($message['content']) }})"></div>
                                                        </div>
                                                        @if (($message['status'] ?? null) === 'cancelled')
                                                            <div
                                                ...
                                                {{-- Meta row --}}
                                                @if (($message['status'] ?? null) === 'completed' && ($message['tokensOut'] ?? 0))
                                                    <div
                                                        ...
                                                        <span
                                                            ...
                                                            <span class="font-mono text-zinc-500 dark:text-zinc-400">{{ number_format((int) $message['tokensIn']) }}</span> in
                                                        </span>
                                                        <span
                                                            class="inline-flex items-center gap-1 rounded-md border border-zinc-200 bg-zinc-50 px-1.5 py-0.5 dark:border-white/10 dark:bg-white/5">
                                                            <span class="font-mono text-zinc-500 dark:text-zinc-400">{{ number_format((int) $message['tokensOut']) }}</span> out
                                                        </span>
                                                        <span
                                                            class="inline-flex items-center gap-1 rounded-md border border-zinc-200 bg-zinc-50 px-1.5 py-0.5 dark:border-white/10 dark:bg-white/5">
                                                            $<span class="font-mono text-zinc-500 dark:text-zinc-400">{{ number_format((float) $message['cost'], 4) }}</span>
                                                        </span>
                                                        <span class="text-zinc-300 dark:text-zinc-600">&middot;</span>
                                                        <span>{{ \App\Support\MockData::timeAgo($message['timestamp']) }}</span>
                                                    </div>
                                                @endif
                                            </div>
                                        </div>
        ...
        <div x-show="showConfig" x-transition:enter="transition ease-out duration-200"
            ...
            <div class="space-y-5 p-4">
                {{-- System prompt --}}
                <div>
                    <button type="button" @click="showSystemPrompt = !showSystemPrompt"
                        ...
                        <flux:icon.chevron-down class="size-3.5 transition-transform"
                            x-bind:class="showSystemPrompt ? 'rotate-180' : ''" />
                    </button>
                    <div x-show="showSystemPrompt" x-transition class="mt-2">
                        <textarea wire:model="system" rows="4"
                        ...
                        @foreach ($providers as $p)
                            <button type="button" wire:click="selectProvider('{{ $p['slug'] }}')"
                                ...
                                @class([
                                    ...
                                    'border-zinc-200 hover:border-zinc-300 dark:border-white/10 dark:hover:border-white/15' => $provider !== $p['slug'],
                                ])>
                                <span class="size-2 shrink-0 rounded-full" style="background-color: {{ $p['color'] }}"></span>
                                <span class="min-w-0 flex-1 truncate font-medium text-zinc-700 dark:text-zinc-200">{{ $p['name'] }}</span>
                                <span class="text-[0.65rem] text-zinc-400">{{ count($p['models']) }} models</span>
                                @if ($provider === $p['slug'])
                ...
                <div>
                    ...
                    <div
                        ...
                        <span class="inline-flex items-center gap-1">
                            ...
                            <span class="font-mono text-zinc-500 dark:text-zinc-400">{{ number_format($currentModel['context'] ?? 0) }}</span> ctx
                        </span>
                        <span class="inline-flex items-center gap-1">
                            <flux:icon.arrow-right-start-on-rectangle class="size-3" />
                            <span class="font-mono text-zinc-500 dark:text-zinc-400">{{ number_format(($currentModel['pricing']['input'] ?? 0), 2) }}</span>
                        </span>
                        <span class="inline-flex items-center gap-1">
                            <flux:icon.arrow-right-end-on-rectangle class="size-3" />
                            <span class="font-mono text-zinc-500 dark:text-zinc-400">{{ number_format(($currentModel['pricing']['output'] ?? 0), 2) }}</span>
                        </span>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        <div
                            ...
                            <p class="font-mono text-sm font-semibold tabular-nums text-zinc-700 dark:text-zinc-200">{{ number_format($totals['tokensIn']) }}</p>
                        </div>
                        <div
                            class="rounded-lg border border-zinc-200 bg-zinc-50 px-3 py-2 dark:border-white/10 dark:bg-white/5">
                            <p class="text-[0.65rem] text-zinc-400 dark:text-zinc-500">Total out</p>
                            <p class="font-mono text-sm font-semibold tabular-nums text-zinc-700 dark:text-zinc-200">{{ number_format($totals['tokensOut']) }}</p>
                        </div>
                        <div
                            ...
                            <p class="font-mono text-sm font-semibold tabular-nums text-brand-600 dark:text-brand-400">
                                ${{ number_format($totals['cost'], 4) }}</p>
                        </div>
                    </div>
                </div>
            </div>
